...
/*
   ...
   on terminal#2: sudo chmod 777 /dev/ttyUSB0
...
Stepper RMotor(steps, 2, 3);
Stepper LMotor(steps, 4, 5);



ros::NodeHandle  nh;

SoftwareDynamixelInterface interface(10, 11, 9); // rx, tx, dir_control
...
ros::Publisher pub1("ArduinoMsg", &ui16_msg);
ros::Publisher pub2("JointsValue", &str_msg);


void messageJoints( const std_msgs::String& msg) {
  motor1.torqueOn(500);
  motor2.torqueOn(500);
  motor3.torqueOn(500);
  motor4.torqueOn(500);
  ...
  motor7.torqueOn(500);
  motor8.torqueOn(500);
  int motor[8]; // use this value to control the arm (AX-Motor)
  String input = msg.data;

  // split the incoming string (msg) and convert to int and publish to topic: ArduinoMsg -->
  char separator[] = ", ";
  char *token;
  ...
  while (token != NULL)
  {
    ...
    pub1.publish( &ui16_msg );
    token = strtok(NULL, separator);
    
  }
  for(int i = 0;i < 8;i++){
  motor[i] = map(motor[i], 0, 360, 0, 1023);
  ...
  motor1.goalPosition(motor[0]);
  motor2.goalPosition(motor[1]);
  motor3.goalPosition(motor[2]);
  motor4.goalPosition(motor[3]);
  motor5.goalPosition(motor[4]);
  motor6.goalPosition(motor[5]);
  motor7.goalPosition(motor[6]);
  motor8.goalPosition(motor[7]);
  
  }
void walker1(const std_msgs::String& msg){
    
    String input1 = msg.data;
    int x,y;
    y = input1.toInt();
   
    if(y>211 && y<588){
       x = map(y, 212, 587, 0, 6000);
       moveBackward(10000);
       delay(3000);
       turnRight(10000);
       delay(3000);
...
void messageCb( const std_msgs::UInt16& msg) {
  ...
  if (msg.data == 9) { // read current pose
    ...
    String msg = str1 + ", " + str2 + ", " + str3 + ", " + str4;
    char cstr[20];
    msg.toCharArray(cstr, 20);
    str_msg.data = cstr;
    //ui8_msg.data = 9;
    //pub1.publish( &ui8_msg );
    ...
    String msg1 = str5 + ", " + str6 + ", " + str7 + ", " + str8;
    char cstr1[20];
    msg1.toCharArray(cstr1, 20);
    str_msg.data = cstr1;
    //ui8_msg.data = 9;
    //pub1.publish( &ui8_msg );
    pub2.publish( &str_msg );
...
ros::Subscriber<std_msgs::UInt16> sub1("setMotor", &messageCb );
ros::Subscriber<std_msgs::String> sub2("joints", &messageJoints );
ros::Subscriber<std_msgs::String> sub3("walker", &walker1 );
