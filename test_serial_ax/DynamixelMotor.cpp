...
void DynamixelMotor::torqueOn(uint16_t aTorque)
{
  if (aTorque == 0) {
    write(DYN_ADDRESS_ENABLE_TORQUE, 0);
    write(DYN_ADDRESS_MAX_TORQUE, 0);
    write(34, 0);
  }
  else
  {
    write(DYN_ADDRESS_ENABLE_TORQUE, 1);
    write(DYN_ADDRESS_MAX_TORQUE, aTorque);
    write(34, aTorque);
  }
